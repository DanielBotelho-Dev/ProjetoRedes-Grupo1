...
Atividade Prática em Grupo — Segurança de Perímetro e Roteamento Avançado
**Arquivo de entrega:** `grupo1.pkt` (Cisco Packet Tracer)
...
```mermaid
flowchart LR
  subgraph ISP["ISP / WAN"]
    ISPEDGE["ISP-Edge (2911)"]:::router
  end
  subgraph BORDA["Borda"]
    RBORDA["R-Borda (2911)"]:::router
  end
  subgraph CORE["Core"]
    RINT["R-Interno (2911)"]:::router
    MLS["MLS-Core (3560-24PS)"]:::switch
  end
  subgraph DMZ["DMZ - 192.168.1.0/24"]
    WEB["Web-DMZ"]:::server
    DNSS["DNS-Server"]:::server
    DHCPS["DHCP-Server"]:::server
  end
  subgraph VENDAS["Vendas - VLAN10 - 192.168.10.0/24"]
    SWV["SW-Vendas (2960-24TT)"]:::switch
    PV["12x PC + Impressora"]:::host
  end
  subgraph ENG["Engenharia - VLAN20 - 192.168.20.0/24"]
    SWE["SW-Engenharia (2960-24TT)"]:::switch
    PE["12x PC + Impressora"]:::host
  end
  ISPEDGE ---|"WAN 200.100.50.0/30"| RBORDA
  RBORDA ---|"10.0.0.0/30"| RINT
  RINT ---|"10.0.0.4/30"| MLS
  MLS ---|"Access VLAN30"| WEB
  MLS --- DNSS
  MLS --- DHCPS
  MLS ---|"Trunk VLAN10"| SWV
  SWV --- PV
  MLS ---|"Trunk VLAN20"| SWE
  SWE --- PE
...
1. Abra `grupo1.pkt` no Cisco Packet Tracer (versão 9.0 ou superior).
2. Aguarde ~30-50 segundos após abrir para o Spanning-Tree convergir antes de testar conectividade.
